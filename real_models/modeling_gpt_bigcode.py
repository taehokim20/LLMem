...
class GPTBigCodeModel(GPTBigCodePreTrainedModel):
    ...
    def forward(
        ...
        print('[cuda:{}][GPTBigCodeModel -> forward]: {} -> {} -> {} -> {} -> {}'.format(local_rank, k, l, m, n, o))
...
class GPTBigCodeForCausalLM(GPTBigCodePreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            d = GPUtil.getGPUs()[0].memoryUsed
            print('[cuda:{}][before loss_fct]: {} -> {} -> {} -> {}'.format(local_rank, a, b, c, d))
            # Flatten the tokens
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
            e = GPUtil.getGPUs()[0].memoryUsed

        if not return_dict:
            output = (lm_logits,) + transformer_outputs[1:]
            return ((loss,) + output) if loss is not None else output

        print('[cuda:{}][GPTBigCodeForCausalLM -> forward]: {} -> {} -> {} -> {} -> {}'.format(local_rank, a, b, c, d, e))
